...
figure(1342);
h = plot(t, R_sen(:,9), t, R_nom(:,9), '-.',  'LineWidth',1 );     
% legend('','nom');
ylabel('{\sl \bf e}_3^T {\sl \bf R}_0  {\sl \bf e}_3');
xlabel('Time (s)'); 
% set(h, 'LineWidth',1);
...
figure(213);
 h = plot(t, t_mag);  grid on; 
ylabel('{\sl \bfT}_{\sl i} (N)','FontName','Times New Roman','FontSize',16); 
...
thrust_vector(3*(ii-1)+1:3*(ii-1)+3);  
    ...
    legend('\phi','\theta', '\psi');
    ylabel('\gamma_d (rad), quadrotor ','FontName','Times New Roman','FontSize',16);
    xlabel('Time (s)','FontName','Times New Roman','FontSize',16);
    set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);
    set(h,   'LineWidth',1);
end

%relative attitude of quadrotor: 
e3re3= zeros(length_t, 6);
for ii = 1:6
thrust_vector(3*(ii-1)+1:3*(ii-1)+3);  

    gamma_sens_quadrotor = zeros(length_t, 3);
    gamma_comr_quadrotor = zeros(length_t, 3);
%     norm_R_array=zeros(length(t),1);
    R_sen_i = y1(:, 9*(ii-1)+19:9*(ii-1)+27);
%     e3re3= zeros(length_t, 1);
    for i =1:length(t)
        R_i = [R_sen_i(i,1), R_sen_i(i,2),  R_sen_i(i,3); R_sen_i(i,4), R_sen_i(i,5), R_sen_i(i,6);  R_sen_i(i,7), R_sen_i(i,8), R_sen_i(i,9)];
        R_0 = [R_sen(i,1), R_sen(i,2),  R_sen(i,3); R_sen(i,4), R_sen(i,5), R_sen(i,6);  R_sen(i,7), R_sen(i,8), R_sen(i,9)];
        R_0_i = R_0'*R_i;
%         norm_R_array(i) = norm(R_i-R_d_i);
        gamma_comr_quadrotor(i,:) = (Euler_angles(R_0_i))';
        e3re3(i,ii) = [0,0,1]*R_0_i*[0; 0; 1];        
    end
    figure(50000+ii); %2-norm of  R-Rd of each quadrotor

    h=plot(t, gamma_comr_quadrotor(:,1), '-.',  t, gamma_comr_quadrotor(:,2), '--', t, gamma_comr_quadrotor(:,3) ); grid;
    legend('\phi','\theta', '\psi');
    ylabel('relative \gamma_d (rad), quadrotor ','FontName','Times New Roman','FontSize',16);
    xlabel('Time (s)','FontName','Times New Roman','FontSize',16);
    set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);
    set(h,   'LineWidth',1);
end
    figure(5012); % relative angle of quadrotor to the load 
    h=plot(t, e3re3(:,1),  t, e3re3(:,2), t, e3re3(:,3), t, e3re3(:,4), t, e3re3(:,5), t, e3re3(:,6) ); grid;
    legend('quadrotor 1','quadrotor 2', 'quadrotor 3','quadrotor 4','quadrotor 5', 'quadrotor 6');
    ylabel('e_3^T R_0^T R_ie_3 ','FontName','Times New Roman','FontSize',16);
    xlabel('Time (s)','FontName','Times New Roman','FontSize',16);
    set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);
    set(h,   'LineWidth',1);
...
gap = floor(length_t/4);
for i =1:gap:length_t
%     R = [R_sen(i,1), R_sen(i,2),  R_sen(i,3); R_sen(i,4), R_sen(i,5), R_sen(i,6);  R_sen(i,7), R_sen(i,8), R_sen(i,9)];
    configuration = [P_sens(i,:)'; R_sen(i,:)'];
    plot_6dpose(configuration);
end

%draw the 3-d position with tubes 
figure(105);    %3-d position
  plot3(P_com(:,1),P_com(:,2),P_com(:,3),'-.',P_sens(:,1),P_sens(:,2),P_sens(:,3)); hold on; 
grid;
plot3(1, 0.05, 5, '*'); 
axis equal;
...
gap = floor(length_t/100);
for i =1:gap:length_t
%     R = [R_sen(i,1), R_sen(i,2),  R_sen(i,3); R_sen(i,4), R_sen(i,5), R_sen(i,6);  R_sen(i,7), R_sen(i,8), R_sen(i,9)];
%     configuration = [P_sens(i,:)'; R_sen(i,:)'];
    R_sph = 0.31; 
    drawsphere(P_nom(i,:),R_sph); 
end

% the position constraints condition:
figure(2002); 
h = plot(t,  sqrt((P_sens(:,1)-1).^2+ (P_sens(:,2)-0.05).^2 + ( P_sens(:,3)-5).^2));
% grid; 
...
subplot(3,1,1);
h = plot(t,P_com(:,1),'-.',t,P_sens(:,1), t,P_nom(:,1), '--');grid;
ylabel('X(m)');
% title('POSITION:SENSED VS COMMAND');
legend('P_{com}','P_{act}', 'P_{nom}');
set(h, 'LineWidth',1);
set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);

subplot(3,1,2);
h = plot(t,P_com(:,2),'-.',t,P_sens(:,2), t,P_nom(:,2), '--' );grid;
ylabel('Y(m)');
set(h, 'LineWidth',1);
set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);

subplot(3,1,3);
h = plot(t,P_com(:,3),'-.',t,P_sens(:,3), t,P_nom(:,3), '--' );grid;
...
subplot(3,1,1);
h = plot(t,gamma_com(:,1)/pi*180,'-.',t,gamma_sens(:,1)/pi*180, t, gamma_nom(:,1)/pi*180, '--'); grid;
ylabel('\phi(\circ)');
% title('Euler angles: Actual VS Commanded');
legend('\gamma_{com}','\gamma_{act}', '\gamma_{nom}');
set(h, 'LineWidth',1);
set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);

subplot(3,1,2);
h = plot(t,gamma_com(:,2)/pi*180,'-.',t,gamma_sens(:,2)/pi*180, t, gamma_nom(:,2)/pi*180, '--'); grid;
ylabel('\theta(\circ)');
set(h, 'LineWidth',1);
set(gca,'FontSize',12, 'FontName','Times New Roman','LineWidth',1.5);

subplot(3,1,3);
h = plot(t,gamma_com(:,3)/pi*180,'-.',t,gamma_sens(:,3)/pi*180, t, gamma_nom(:,3)/pi*180, '--'); grid;
