...
//the position, velocity, and input force are all expressed in earth frame
    ...
    double delta_x = 5*p_err + 2*v_err + 1*yaw_err; //the coefficient of the volocity can also be adjusted. 	
 
    Eigen::Vector3d u_gen_trans;
    u_gen_trans << 0, 0, 0;   //The force is expressed in earth frame, and does not include the gravity force. 
     
	double delta_u =  (f_x - u_gen_trans(0) )* (f_x - u_gen_trans(0) )+ (f_y - u_gen_trans(1))* (f_y - u_gen_trans(1))  + (f_z - u_gen_trans(2))* (f_z - u_gen_trans(2)) + 1*psi_dot*psi_dot; 
    ...
    f[7] = 3*delta_x + 0.01*delta_u;   
    f[7] = 10*delta_x + 0.05*delta_u;  
    f[7] = 3*delta_x + 0.1*delta_u;  
